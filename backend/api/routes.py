...
Each endpoint maps directly to an existing Game method:
    ...
    game.add_team(...)           →  POST   /games/{game_id}/teams
    game.set_tires(...)          →  PUT    /games/{game_id}/tires
    game.set_starting_positions  →  PUT    /games/{game_id}/positions
    driver_turn (one-by-one)     →  POST   /games/{game_id}/turns
    all_at_once + apply          →  POST   /games/{game_id}/turns/aao
    check_positions / standings  →  GET    /games/{game_id}/standings
    get_driver_state             →  GET    /games/{game_id}/drivers/{name}
    —                            →  GET    /games/{game_id}
    —                            →  GET    /games
...
# PUT /games/{game_id}/tires
class SetTiresRequest(BaseModel):
    name: str
    tire: str

# PUT /games/{game_id}/positions
...
def create_game(body: CreateGameRequest) -> CreateGameResponse:
    ...
    return CreateGameResponse(id=game_id, track=body.track, weather=body.weather)
...
def add_teams(game_id: str, body: AddTeamsRequest) -> Dict[str, Any]:
    """
    ...
    for t in body.teams:
        ...
        try:
            team = game.add_team(t.name, t.driver1, t.driver2, tires)
            added.append({
                'team': t.name,
...
def set_tires(game_id: str, body: SetTiresRequest) -> Dict[str, Any]:
...
def set_positions(game_id: str, body: SetPositionsRequest) -> Dict[str, Any]:
...
def execute_turn(game_id: str, body: TurnRequest) -> Dict[str, Any]:
...
def all_at_once_roll(game_id: str, body: AllAtOnceRequest) -> Dict[str, Any]:
...
def all_at_once_place(game_id: str, body: AllAtOnceLocationRequest) -> Dict[str, Any]:
...
def get_standings(game_id: str) -> List[Dict[str, Any]]:
...
def get_driver(game_id: str, name: str) -> Dict[str, Any]:
    """Full state of a single driver."""
    game = _get_game(game_id)
    try:
        return game.get_driver_state(name)
    except ValueError as  e:
        raise HTTPException(status_code=404, detail=str(e))